...
function AssignStartingPlots:AddExtraBonusesToHillsRegions()
	...
	for loop, region_number in ipairs(shuffled_hills_regions) do
		...
		if table.maxn(flat_tropical_plains) > 0 then
			...
			self:ProcessResourceList(32 / infertility_quotient, ImpactLayers.LAYER_BONUS, flat_tropical_plains, resources_to_place);
		end
		if table.maxn(flat_tropical_grass) > 0 then
			local resources_to_place = {
			{self.rice_ID, 1, 100, 0, 2} };
			self:ProcessResourceList(28 / infertility_quotient, ImpactLayers.LAYER_BONUS, flat_tropical_grass, resources_to_place);
		end
...
function AssignStartingPlots:PlaceBonusResources()
	...
	-- Even More Resources for VP end
		...
		self:ProcessResourceList(24 * resMultiplier, ImpactLayers.LAYER_BONUS, self.rice_list, resources_to_place)
		
		resources_to_place = {
		{self.maize_ID, 1, 100, 0, 2} };
		self:ProcessResourceList(32 * resMultiplier, ImpactLayers.LAYER_BONUS, self.maize_list, resources_to_place)
